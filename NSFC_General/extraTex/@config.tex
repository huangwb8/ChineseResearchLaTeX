...
\ifwindows
  ...
  \NewDocumentCommand \templatefont   { } {\kaitichar } %使用模板字体代替直接使用楷书
  \setmainfont{Times New Roman} 
  \setCJKmainfont{KaiTi}[AutoFakeBold=3] 
  \xeCJKsetup{PunctStyle=quanjiao} % 强制中文标点符号使用中文字体
  % \setCJKmainfont{SimSun}[AutoFakeBold=3] % 可参考[LaTeX 中文字体配置基础指南 - 知乎](https://zhuanlan.zhihu.com/p/538459335) 选择自己喜欢的字体。 个人感觉中宋（SimSun）还可以 (～￣▽￣)～ 
\else
  \newfontfamily\kaitichar[Path=./fonts/, AutoFakeBold=3, PunctuationSpace=0]{Kaiti.ttf} % 设置楷体字体，指定字体文件的路径
  \NewDocumentCommand \templatefont   { } { \kaitichar } % 使用模板字体代替直接使用楷书
