...
def test_docx_issue_form_requires_reference_docx_context():
    form = load_yaml(ISSUE_TEMPLATE_DIR / "docx-template-support.yml")

    assert form["name"] == "DOCX 模板问题 / 需求"
    assert form["title"].startswith("[DOCX]")

    source_item = get_body_item_by_id(form, "docx_template_source")
    assert source_item["type"] == "textarea"
    assert source_item["validations"]["required"] is True
    assert "DOCX 模板文件如何获取" in source_item["attributes"]["label"]
    assert "reference.docx" in source_item["attributes"]["placeholder"]

    ack_item = get_body_item_by_id(form, "docx_material_ack")
    options = ack_item["attributes"]["options"]
