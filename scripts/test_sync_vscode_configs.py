...
def test_gdnsf_regional_young_project_uses_the_gdnsf_profile_and_wrapper():
    project_dir = REPO_ROOT / "projects" / "GDNSF_Regional_Young"

    assert syncer.infer_project_profile(project_dir.name) == "gdnsf"
    ...
    assert "scripts/gdnsf_build.py" in settings
