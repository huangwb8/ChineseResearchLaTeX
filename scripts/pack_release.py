...
def detect_thesis_template_id(project_dir: Path) -> str:
    ...
    if extratex_dir.is_dir():
        search_files.extend(sorted(extratex_dir.rglob("*.tex")))
    for tex_file in search_files:
...
def build_overleaf_shell_escape_latexmkrc() -> str:
    """生成 Overleaf 专用 latexmk 配置，用于需要 minted 的模板。"""
    return "\n".join(
        [
            "# Auto-generated by scripts/pack_release.py for Overleaf release bundles.",
            "$pdf_mode = 5;",
...
def project_needs_overleaf_shell_escape(project_dir: Path) -> bool:
    """判断 Overleaf 包是否需要通过 latexmkrc 开启 shell escape。"""
    return detect_project_kind(project_dir) == "thesis" and detect_thesis_template_id(project_dir) == "thesis-ucas-doctor"
...
def build_paper_runtime_bundle(runtime_dir: Path, project_dir: Path) -> None:
    ...
    for file_name in PAPER_SHARED_RUNTIME_FILES:
        ...
        if file_name == "bml-core.sty":
            content = file_path.read_text(encoding="utf-8")
            content, profile_rewrite_count = re.subn(
                r"\\InputIfFileExists\{\\bml@packageroot%\s*profiles/bml-profile-\\bml@template\.def\}\{\}\{%",
                lambda _: r"\InputIfFileExists{styles/profiles/bml-profile-\bml@template.def}{}{%",
                content,
            )
            if profile_rewrite_count != 1:
                raise RuntimeError("无法重写 bensz-paper Overleaf profile 路径")
            for module_name in (
                "bml-layout",
                "bml-headings",
                "bml-typography",
...
def populate_overleaf_bundle(bundle_dir: Path, project_dir: Path) -> None:
    ...
    if project_needs_overleaf_shell_escape(project_dir):
        write_text_file(bundle_dir / ".latexmkrc", build_overleaf_shell_escape_latexmkrc())
