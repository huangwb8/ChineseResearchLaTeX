...
% 标题左缩进：对齐 2026 Word 基准 PDF（约 28pt）
\newlength{\NSFCTitleIndent}
\setlength{\NSFCTitleIndent}{28pt}

% 二级提纲（蓝色提示语）的首行缩进宽度（对齐 Word：首行起始点较左边距右移约 28pt）
\newlength{\NSFCSubsectionIndent}
\setlength{\NSFCSubsectionIndent}{28pt}

% 二级提纲（蓝色提示语）段后距：用于控制 \NSFCSubsection 与下一段之间的间隙
% 说明：原先为 22pt（更贴近 Word 的“空段落”观感），但在 LaTeX 排版下显得偏松，改为更紧凑的默认值。
\newlength{\NSFCSubsectionAfterSkip}
\setlength{\NSFCSubsectionAfterSkip}{10pt}
...
\newcommand{\NSFCSubsection}[1]{%
  ...
  {%
    \color{MsBlue}\subsectionzihao\templatefont
    \noindent%
    % 使用 parshape 明确控制首行/后续行缩进：首行右移 28pt，后续行回到左边距
    \parshape 2 %
      \NSFCSubsectionIndent \dimexpr\linewidth-\NSFCSubsectionIndent\relax %
      0pt \linewidth %
    #1\par%
  }%
  % 提纲段落后的间隙（可通过 \NSFCSubsectionAfterSkip 统一调整）
  \vspace{\NSFCSubsectionAfterSkip}%
}
...
\titlespacing*{\section}
  {\NSFCTitleIndent} % Left indentation
