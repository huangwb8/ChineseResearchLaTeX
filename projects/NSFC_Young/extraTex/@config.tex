...
\geometry{left=3.28cm,right=3.28cm,top=2.67cm,bottom=3.16cm} % 页边距（参考 4543de7 的版心宽度设置）
\CJKsetecglue{\hskip 0.5em plus 0.05em minus 0.1em} % 数字与CJK字符的间距（对齐 Word 2026 PDF）
% left=3.28cm,right=3.28cm
...
\addto\captionsenglish{
    ...
    \renewcommand{\refname}{\sihao \templatefont \bfseries \leftline{参考文献} \vspace*{5mm}} %四号字，楷体，居左
...
\ExplSyntaxOn
\cs_new_protected:Npn \nsfc_subsection_before:n #1
  {
    % Word 2026 模板中：括号提示行（如“（为什么要…）”）缩进略小于编号行
    \regex_match:nnTF { \A \s* （ } {#1}
      { \hspace*{1.5em} #1 }
      { \hspace*{2em} #1 }
  }
\ExplSyntaxOff

\newcommand{\NSFCSubsectionBefore}[1]{%
  \csname nsfc_subsection_before:n\endcsname{#1}%
}
...
\titleformat{\subsection}
  ...
  {} % label
  {0pt} % separation
  {\NSFCSubsectionBefore}   % before-code（用于缩进，不影响标题文字内容）
...
\titlespacing*{\subsubsection}
  {0pt} % Left indentation
  {0pt plus 0pt minus 0pt} % Space before the title
  {5pt} % Space after the title

% Define line stretch for subsubsection
\titlespacing*{\subsubsubsection}
  {0pt} % Left indentation
  {0pt plus 0pt minus 0pt} % Space before the title
  {7.8pt} % Space after the title
