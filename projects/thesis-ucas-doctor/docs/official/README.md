...
本目录用于保存 `thesis-ucas-doctor` 的官方资料、来源说明与完整性校验值。
...
- 本目录是 UCAS 资环官方 Word 参考文件的项目内单一位置；不在项目根目录或其他目录保留副本，避免多处拷贝漂移。
