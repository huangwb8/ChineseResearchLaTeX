...
description: 去除 NSFC 标书中的 AI 机器味，尤其适用于模板化转折、抽象概念堆叠、边界声明过重、研究动作不清的标书段落，使文本读起来像资深领域专家亲笔撰写（不适用：非标书内容/需修改格式/需补充新内容）
metadata:
  author: Bensz Conan
  keywords:
    - nsfc-humanization
...
- 遇到“不是 A 而是 B”“不仅 A 而且 B”“既 A 也 B”这类整齐二分结构时，先判断其是否承载真实信息；若只是概念归类，应改成更顺的过程句
...
5. 逐行润色可编辑片段，必要时把过重的括号信息改写为正常句流，把概念定位句改写为原文事实支持的研究动作句。
6. 逐行自检结构是否保持，受保护片段是否逐字一致。
7. 做 1-2 轮风格自评，仍以“零损失”优先；特别检查是否误把“更具体”写成了“新增事实”。
8. 按 `output_mode` 输出文本、摘要和可选 STYLE_CARD。
