...
工作 map 保留完整研究线、Search record ID、证据深度和稳定 O/R 编号。正式摘要从真实问题进入，解释核心术语、主要答案、研究线关系、最强近邻和重要未知，再引出 O 机会；表格和时间线按需要使用。成稿后人工复核就近证据、反方证据、来源结论与本轮推断，以及摘要级材料的表述强度。

新报告使用 `research-idea-report-v3`。默认按 GB/T 7714—2025 顺序编码制使用 `[1](#ref-1)` 引注，`reference_map` 将工作 R 编号映射到书目编号，最后一个二级小节为 `## References`。指定期刊或学位样式可声明 `citation_style: custom` 和规范来源，仍须逐条维护映射并人工复核。旧 v2 R 锚点报告可只读校验，不能用于新运行完成。
