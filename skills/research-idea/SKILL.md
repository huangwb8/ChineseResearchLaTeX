...
- 工作区：用户指定时遵从；未指定时为当前工作目录下 `.research-idea/`。
- 轮次：默认 3 轮；用户指定时遵从。
...
报告不得暴露 `.research-idea/`、`tests/research-idea/`、`parallel-vibe/`、`.parallel_vibe/`、`@main/summary.md`、manifest 或其他中间产物路径。

## 工作区

- 默认工作区：`{cwd}/.research-idea/run-{timestamp}/`。
...
3. 调用 `research-literature-review`，档位固定为 `Premium`，输出目录限定为 `.research-idea/run-*/novelty/Cx/`。
