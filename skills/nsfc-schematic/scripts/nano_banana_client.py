...
def _choose_aspect_ratio(w: int, h: int) -> str:
    ...
    """
    # API expects a string like "16:9" (not enum names).
    ratios = {
        "16:9": 16 / 9,
        "9:16": 9 / 16,
        "4:3": 4 / 3,
...
def nano_banana_generate_png(
    *,
    ...
    # Best-effort retry for transient quota/rate-limit errors.
    attempts = 0
    ...
    while attempts < 5:
        ...
        resp = {}  # unreachable, but keeps type checker happy
    ...
    if best is None:
        # Surface a small excerpt for debugging, but keep it short.
