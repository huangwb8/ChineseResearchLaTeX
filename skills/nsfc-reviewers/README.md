...
- 会识别“受资助额度限制的设计妥协”，避免把预算约束误判为纯粹能力问题
- 跨组共识识别 + 最小修改序列
...
- **真正的设计错误**：逻辑不成立、假说不可检验、证据链断裂，这类问题会直接影响送审判断。
- **受资助额度限制的现实妥协**：例如青年基金常见 `30–40w`、面上项目常见 `50–60w` 的资助框架下，研究内容只能聚焦单一主问题、样本/平台/验证层级只能做到“最小可行闭环”。

当技能判断某个短板与资助额度有关时，报告会：

- 明确写出“受基金所限，此处设计偏弱/存在缺陷”
- 解释为什么这是现实约束下的妥协，而不是简单的“不会设计”
- 同时补充“若资助不受限，一个更完整的设计应如何展开”

这条规则**不会替当前版本免责**：函评/会评判断仍然只看“如果今天就提交，能不能过”。
...
- **建议**：可执行的修改方案
- **验证**：改完如何自检
- **资助约束说明（如适用）**：说明该缺陷是否与青年/面上等有限资助额度有关
- **完整设计参考（如适用）**：若资助不受限，一个更完整的研究设计应包含什么
...
### Q：为什么报告会说“受基金所限，此处设计偏弱”？

因为青年基金、面上项目的资助额度本来就有限。某些实验链条、样本规模或平台建设不够“豪华”，未必是申请人不会设计，而可能是预算约束下的现实取舍。技能会把这种情况单独标出来，同时告诉你“如果经费不受限，完整版设计应长什么样”。
...
- `funding_context`：资助额度约束识别规则，以及“受限妥协”如何写入报告
