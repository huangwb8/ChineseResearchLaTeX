...
**核心价值**：
- 7 位专家角色（创新性/可行性/基础/严格/建设性/科学意义/清晰度）独立评审
- 支持多组并行评审（默认 3 组、最多 5 组，共 21–35 人次）
- 默认给出“基于当前版本直接送审”的函评/会评二元判断（给过 / 不给过）
- 问题分级（P0 致命 → P1 重要 → P2 建议）+ 证据锚点
- 跨组共识识别 + 最小修改序列

**重要声明**：本技能输出仅用于写作改进与自查；其中“函评/会评给不过”属于当前版本送审风险预估，不代表任何官方评审口径，也不构成资助结论。
...
```
使用 nsfc-reviewers skill 开 3 组评审团对本标书进行评审，按默认规则将结果保存到标书目录
```
...
- **默认组数**：3 组（21 位专家人次）
- **最大组数**：5 组（35 位专家人次）
- **每组专家**：7 位（创新性/可行性/基础与团队/严格综合/建设性/科学意义/清晰度）
- **依赖技能**：`parallel-vibe`（若不可用会自动退化为单组模式）

**成本参考**：
- 每组评审约相当于 7 位独立专家各完成一次完整评审
...
4. **严格综合评审专家**：全局视角，更挑剔
5. **建设性评审专家**：关注改进空间
6. **科学意义与领域影响专家**：重要性、产出牵引力
7. **可读性与论证清晰度专家**：邻域可理解性、结构清晰度
...
### Q：为什么现在会直接写“函评给不过 / 会评给不过”？

这是为了让你对当前版本的送审风险有更直观的判断。技能会强制基于“如果今天就提交”来做二元结论，并配上把握度与关键依据；如果判“不过”，还会指出最优先的翻盘修改点。
...
- `stage_assessment`：函评/会评判断规则、标签与把握度口径
