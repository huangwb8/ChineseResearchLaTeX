...
- `core/hybrid_coordinator.py`：协同编排（把各模块串成闭环）
- `core/diagnostic.py`：Tier1 诊断（结构/引用/字数/禁用表述与命令）
- `core/writing_coach.py`：渐进式引导（阶段判断 + 可复制提示词 + AI fallback）
- `core/security.py`：白名单写入策略（拒绝写入 main.tex/.cls/.sty 等）
- `core/editor.py` + `core/latex_parser.py`：按 `\\subsubsection{...}` 精确替换正文
- `core/versioning.py`：runs 备份、diff、rollback
- `core/html_report.py` + `templates/html/report_template.html`：HTML 可视化报告
- `core/example_matcher.py` + `examples/**`：示例推荐（支持 `*.metadata.yaml`）
- `core/config_loader.py`：配置加载（基础配置 + preset + 用户 override）
