...
> 本目录是 `transfer-old-latex-to-new` 的“拆分文档”。核心入口仍是 `scripts/run.py`。
...
# 查看某次 run 的关键产物路径
python skills/transfer-old-latex-to-new/scripts/run.py runs show --runs-root /path/to/runs --run-id <run_id>
```
