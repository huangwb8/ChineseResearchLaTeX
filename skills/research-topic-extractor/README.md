# Research Topic Extractor — 用户使用指南

本 README 面向**使用者**：教你如何触发并正确使用 `research-topic-extractor` skill。

如果你在编辑/维护该 skill：执行指令与强制规范在 `research-topic-extractor/SKILL.md`。

## 旧名兼容

- 当前正式名：`research-topic-extractor`
- 旧名：`get-review-theme`
...
`research-topic-extractor` 能够从各种输入源中提取结构化的综述主题，包括：
...
### 示例 9：结合 research-literature-review 使用

```
用户：分析 /path/to/proposal.txt 提取主题，然后用 research-literature-review 做标准级综述
```
...
## 与 research-literature-review 的集成

### 场景 1：单步传递主题

```
用户：从 /path/to/grant-proposal.txt 提取主题，然后用 research-literature-review 做综述

AI 执行流程：
1. 调用 research-topic-extractor 分析文本
2. 获取结构化主题
3. 提取"主题"字段
4. 传递给 research-literature-review
5. 执行文献综述流程
```
...
```
用户：从 research-plan.md 提取主题（含关键词和核心问题），然后用 research-literature-review 做旗舰级综述

AI 执行流程：
1. 调用 research-topic-extractor，请求 JSON 格式输出
2. 解析 JSON，获取 topic、keywords、core_questions
3. 将 topic 作为主题
4. 将 keywords 补充到检索策略
5. 将 core_questions 作为研究范围参考
6. 传递给 research-literature-review
```
