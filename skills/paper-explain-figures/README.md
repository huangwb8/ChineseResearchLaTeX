...
- 不会修改你的 figure 文件
- 不会修改你的源代码文件
- 中间产物只写入当前目录下 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures/`
- runner 与图片转换器的 `HOME/TMP/XDG` 运行时辅助文件也会被重定向到 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures/`
...
| `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures/` | 中间产物目录（日志、转换图、单图解读等） |
...
- `shell` runner 已禁用：它无法对“.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures 之外绝不泄露中间文件”提供严格保证
- `codex` / `claude` / `local` 会在 job 目录下执行，并把 HOME、TMP、XDG cache/state/config 等运行时目录重定向进 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures/`
- 脚本结束前会审计当前工作目录；若发现 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-explain-figures/` 外新增了非结果文件，会自动清理并报错/告警
