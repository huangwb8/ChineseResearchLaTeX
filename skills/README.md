...
### 4. nsfc-justification-writer - 理论创新导向的立项依据写作
...
- **API 缓存（可选）**：默认关闭；需要可复现调试/减少重复请求时再开启
