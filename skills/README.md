...
说明：
- 所有会产生中间文件的 skill 默认遵循 [WORKSPACE.md](WORKSPACE.md)：同一任务只使用一个 `./.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/` 根目录，并在 `{skill名}/input|output|log/` 中隔离过程产物；旧隐藏目录仅作显式兼容读取、迁移或清理。
...
**功能**：面向各类科研基金申请书“立项依据”章节的语义写作、逻辑审查与专业可读性复核；以准确性优先，帮助大同行低负担追踪主张—证据—缺口—切入点，不把专业内容改成科普文

**使用场景**：需要把“价值与必要性、现状不足、科学问题/科学假设、切入点与贡献”自动写成正文并安全落盘；支持自定义文件名、标题宏和 LaTeX 结构，默认自动备份、写入并输出 unified diff

**专业可读性复核**：面向熟悉本学科但未必熟悉该细分方向的大同行，定位长句层级、指代/缩写界定、抽象名词关系与段内衔接，并给出保留事实、术语、必要限定、引用命令和结构的保真改法；已清楚的专业表述无需为通俗而改写

**能力**：
- Tier1 确定性诊断与宿主 AI Tier2 语义复核
- 缺失信息的保守假设、自动成稿和正文-only 自动重试
- 自动目标选择、白名单写入、备份、diff 与回滚
...
```
请用 research-literature-review 写一篇"HER2-ADC在乳腺癌中的研究进展"的Premium级综述。参考文献以近2023-2025年为主，更早之前的文献，如果特别相关、特别重要的，也可以纳入。要有一个小节专门讨论未来3年较有前景的研究方向，并将 PDF/Word 发布到 `./review-deliverables/`。
```

**技能特点**：
- **显式多查询契约**：AI 先生成并保存 5–25 条查询；缺失、冲突或无效时默认停止，不静默改跑单查询
- **可审计后备**：只有显式授权才执行一次单查询，并在 state/Search Log 中记录来源、原因与警告
...
**功能**：基于 NSFC 标书正文或其它材料，生成预算说明书 LaTeX 项目并渲染 `budget.pdf`；所有中间过程默认隔离到工作目录 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/nsfc-budget/{yyyy-mm-dd-hh-mm}/` 下。
...
总预算：50w
要求：输出 LaTeX 项目与 budget.pdf；中间文件全部进入 .bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/nsfc-budget/{yyyy-mm-dd-hh-mm}
```
...
- 所有中间文件默认收纳到 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-select-journal/{yyyy-mm-dd-hh-mm}/` 隐藏工作区
...
- 默认把中间文件隔离到 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/paper-know-journal/{yyyy-mm-dd-hh-mm}/`，最终交付 `KnowJournal-{杂志名}.md`
...
- 默认使用“交付目录 + `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/nsfc-qc/{yyyy-mm-dd-hh-mm}/` 工作区”隔离中间文件
...
- 默认将报告写入 `.bensz-api/task-{yyyymmdd-hhmm}-{简短描述}/nsfc-length-aligner/{yyyy-mm-dd-hh-mm}/` 隐藏工作区，避免根目录污染
...
**功能**：在不新增信息、不破坏 LaTeX 结构的前提下，按词语、句法、段落和章节四层去除 NSFC 标书“机器味”，识别工程协议腔、规格书字段串、术语漂移、跨章节重复，并审计安全不变量

**使用场景**：
- 正文已经基本成型，但表达发硬、句式过于模板化
- 段落反复出现“不是……而是……”“不只是……更是……”等整齐二分句式
- 段落大量使用概念定位和泛化功能词，研究动作不够清楚
...
**技能特点**：
- 核心原则是“语义零损失 + 结构保护”
- 兼容纯文本与 LaTeX 混合文本，保护代码状态、数字、阈值、分母和失败处理
- 通过术语表和“保留/改写/合并/人工确认”分类降低误报
- 支持把抽象定位句改写为原文事实支持的研究动作句
- 支持 `minimal / moderate / aggressive` 三档润色强度
- 可选输出分层诊断、变更摘要与 STYLE_CARD，便于跨段落保持风格一致
