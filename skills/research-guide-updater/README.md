# Research Guide Updater — 用户使用指南

本 README 面向**使用者**：如何触发并正确使用 `research-guide-updater` skill。
执行指令与硬性规范在 `SKILL.md`；默认参数在 `config.yaml`。

## 旧名兼容

- 当前正式名：`research-guide-updater`
- 旧名：`guide-updater`
...
```
research-literature-review（文献综述） → research-guide-updater（优化指南）
                   ↓
      在与 AI 的持续交互中明确标书关键部分，
      深化对项目的理解，形成可复用的写作规范
```

这是 `research-guide-updater` 最强大的使用场景——先用 `research-literature-review` 进行文献综述，然后将综述中沉淀的洞察融会贯通，有机地、增量地优化项目指南。
...
通过多轮对话逐步沉淀写作洞见，让 `research-guide-updater` 在交互过程中持续优化项目指南：

```
我后续会给出很多指令，我需要你结合那些指令和 research-guide-updater 这个skill来进行
...
```
我之前用 research-literature-review 这个skill做了很多文献综述的工作，
保存在 xxx/reviews 这个文件夹里，其中和目前标书相关的主题包括：xxx、xxx。
请仔细阅读这些综述的 tex 文件（只读，千万不能修改），彻底理解它们的内容，
融会贯通，然后使用 research-guide-updater 这个skill 有机地、增量地优化本项目的
`项目指南.md`。如果`项目指南.md`不符合 research-guide-updater 的一般规范，可以修改，
但千万不要修改该skill的源代码。
```
...
`research-guide-updater` 的核心价值在于：
...
```bash
# 检查指南文件是否包含所有必需的标题
python3 skills/research-guide-updater/scripts/validate_guide.py projects/MyProject/项目指南.md
```
...
A：不会。`research-guide-updater` 遵循"融入式更新"原则：
- 不新增 `##` 级标题
- 不改动既有标题的文本、编号、顺序
- 不为"整理美观"重排大量内容
- 只在既有小节/列表中新增 bullet
...
```bash
python3 skills/research-guide-updater/scripts/validate_guide.py <指南文件路径>
```
