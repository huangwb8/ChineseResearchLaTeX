...
- SCI 正文尽量少用冒号驱动的解释句，例如 `Key issue: ...`、`This indicates: ...`、`The reason is: ...`
- 冒号只在确有结构需要时使用，例如 Figure legend 的 panel 标签、列表引入、方法字段、期刊规定格式或术语定义
- 普通论述优先改成自然从句、分号或独立句，让句子像论文叙事，而不是口头汇报提纲
- 同一段内不要连续出现多个冒号句；如果冒号只是为了把一句话拆成“标题 + 解释”，通常应改写为完整句
- `Introduction` 中的核心观点、核心痛点和核心问题只在最合适的位置充分展开一次；后续出现必须承担推进、限定或收束功能
- 若后文只是换词重复同一痛点，应合并、删除或改为新的证据层级，而不是反复提醒读者“问题很重要”
...
- 结尾明确本文解决什么问题、贡献是什么
- 核心痛点不要在背景、局限和贡献段反复用近义句重说；背景负责引出，局限负责聚焦，贡献段负责回应
- 每段只完成一个推进任务，避免每段都回到同一句“目前仍缺乏……”
- 结尾段可回扣核心问题，但应转为“本文如何回答”，不要再从头重述痛点
