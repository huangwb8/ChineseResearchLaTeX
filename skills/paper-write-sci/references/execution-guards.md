# Execution Guards

这份文档承接 `paper-write-sci` 的长规则块。`SKILL.md` 只保留触发条件、主流程、边界和调度关系；需要判断具体写法、审查口径或通过标准时，再按需读取这里的对应章节。

## 何时读取本文件

- 需要判断某一节“应该做什么”，而不只是“怎么润色”
- 需要执行 `section-role-check` 或 `Discussion audit`
- 需要处理 `.tex` 正文可读性
- 需要审查数字、缩写或逻辑树
- 需要确认全文一致性终审的通过标准

## 章节职责守卫

任何章节优化都先判断“这一节应该做什么”，再判断“应该写成什么风格”。

- `Introduction`：提出问题、界定空白、给出研究目标；不要提前泄露结果性结论
- `Introduction` 的核心观点、核心痛点和核心问题应各有唯一主展开位置；不要在背景段、局限段和贡献段反复用近义句重说
- `Methods`：说明如何回答问题；不要把结果或解释偷写进方法
- `Results`：报告事实、比较和证据锚点；不要提前做 `Discussion` 式意义阐释
- `Discussion`：解释意义、边界、文献关系、机制或临床转化价值；不要把上一节压缩重写一遍
- `Conclusion`：收束贡献与边界；不要引入新证据

对 `Discussion` 额外执行以下硬约束：

1. 若出现连续的 figure-level 结果复述，优先判定为结构性缺陷，而不是“更具体”
2. 默认每段最多保留 `1` 个核心定量锚点，且该锚点必须直接服务于后续解释
3. 不要追求覆盖所有主图结果；只保留不可替代、能支撑讨论动作的少量结果锚点
4. 每段优先完成以下任务之一：解释主要发现、对位既有文献、说明临床/机制意义、交代局限与适用边界、提出未来验证路径

## 写作节奏守卫

SCI 正文应呈现连续论证，而不是口语化提纲。

- 少用冒号解释句。`X: Y`、`This suggests: ...`、`The key problem is: ...` 这类结构容易让论文显得像汇报稿或 AI 提纲
- 冒号保留给确有结构功能的场景，例如 Figure legend 的 panel 标签、列表引入、方法字段、术语定义或期刊模板要求
- 普通论述优先改成自然从句、分号或独立句；如果冒号前只是一个抽象标签，通常应删掉标签并把信息并入句子
- 全文终审时检查段首、段尾、贡献句和 Figure legend 之外的冒号密度；连续多个冒号句通常需要改写

对 `Introduction` 额外执行去冗余检查：

1. 列出核心观点、核心痛点和核心问题在各段出现的位置
2. 判断每次出现是否承担新功能：引出背景、界定空白、限定研究对象、提出目标或收束贡献
3. 若只是近义重复，优先合并到第一次主展开处，或改成新的证据、边界、研究对象限定
4. 结尾段回扣核心问题时，应转向“本研究如何回答”，不要再次铺陈同一痛点

## 章节职责审查

除数字审查与逻辑审查外，还必须单独检查“这一节是否在做它该做的事”。

### 必答问题

每轮 `section-role-check` 至少回答：

1. 当前章节是否承担了错误职能
2. 是否存在“把上一节压缩重写一遍”的倾向
3. `Introduction` 是否反复重提同一核心观点、痛点或问题，而没有形成新的逻辑推进
4. 是否出现“数字都对，但章节功能错位”的问题
5. 对 `Discussion` 而言，解释、文献、局限、未来方向的比重是否高于结果复述

### 审查机制

- 使用 `parallel-vibe` 做独立并行审查
- 并发参数与 SDK 以 `config.yaml:section_role_check.parallel_vibe` 为准
- 审查记录写入当前运行目录的 `section-role-check/`
- 通用模板使用 `references/templates/section-role-check-template.md`
- 若任务聚焦 `Discussion`，或用户明确表达“讨论像结果”“减少重复”“更像人写的 discussion”等意图，必须额外触发 `Discussion audit`
- `Discussion audit` 使用 `references/templates/discussion-role-check-template.md`
- 若任一 runner 指出阻塞性章节串位问题，必须先修复，再进入逻辑终审

### `Discussion audit` 触发条件

出现以下任一意图时，强制触发 `Discussion audit`：

- “优化讨论”
- “讨论像结果”
- “discussion 太像 results”
- “减少重复”
- “更像人写的 discussion”

专项审查输出至少包含：

1. 复述型句子列表
2. 空心评论句列表
3. 缺失的解释任务列表
4. 建议保留的少量定量锚点

## `.tex` 可读性：分段分点

编辑对象是 `.tex` 正文文件，因此必须同时优化“渲染后阅读体验”和“源文件回看体验”。

### 分段

- 需要另起一段时，必须在文本块之间保留 `1` 个或以上空行；在 `.tex` 中，空行意味着新段落
- 典型触发场景：背景切到空白、目的切到方法、结果切到解释、一个局限簇切到另一个局限簇、总述切到图注导读
- 不要为了视觉整齐随意插空行；一旦插空行，就等于明确告诉读者“这里已经是新段落”

### 分点

- 多个点仍属于同一段时，不要插空行；改为让每个“并列信息单元”各占一个物理行，保持“同段但分点”的 `.tex` 源文结构
- 这里的“点”优先指：并列证据单元、并列 panel、并列比较项、并列局限项，而不是任意一个普通句子
- 典型触发场景：同一段内串联多个 Figure/Table 证据、多个组别/亚型对比、panel-by-panel legend、同一段内连续展开的观点或局限
- 同一物理行优先只承载一个便于人类定位的点，这样用户从 PDF/Word 点击回源文时，更容易直接落到目标位置

### 使用边界

- 不要机械地把每一句都拆成单独一行
- 不要把“分点换行”误用成真正列表；只有论文体裁本身适合列表时，才考虑显式 list 环境
- 只对纯正文自然语言段落使用；不要主动重排行敏感结构，例如 `%` 注释拼接、宏参数、大括号内文本、命令密集行、环境头尾或其他可能影响编译、空白语义的 LaTeX 结构
- 不要为了套用新规则而重排整节或整文件；仍应遵循最小必要修改原则

## 数字审查

任何新增、改写或重述的数字，都必须在写入正文前经过数字审查。数字包括但不限于：

- 样本量
- 百分比
- `P` 值
- `HR`、`OR`、`RR`、`AUC`、`C-index`
- 均值、标准差、置信区间
- 随访时长、阈值、版本号

### 必查问题

1. 数字是否来自真实存在的材料
2. 数字是否在当前句子里用得合适
3. 数字的解释是否合理且不越界

### 审查机制

- 使用 `parallel-vibe` 做独立并行审查
- 并发参数与 SDK 以 `config.yaml:number_check.parallel_vibe` 为准
- 每个 runner 都必须覆盖上述三项检查
- 若任一 runner 给出阻塞性问题，则该数字不得写入正文
- 审查记录写入当前运行目录的 `number-check/`
- 审查模板使用 `references/templates/number-check-template.md`

### 数字写入铁律

- 不允许凭经验补数字
- 不允许从图感受、肉眼估计或语义猜测生成精确值
- 不允许把一个材料里的数字套用到另一个语境
- 不允许把统计显著性误写成效果大小，或反过来

## 全文缩写一致性守卫

缩写检查默认是“整篇论文级”，不是“当前正在编辑的单个 tex 文件级”。

### 作用域

- 作用域以 `config.yaml:abbreviation_check.scope` 为准，默认覆盖本轮论文项目中所有可编辑、承载正文语义的 `.tex`
- 至少覆盖 `config.yaml:abbreviation_check.include_section_paths` 中列出的 `abstract`、正文各节、`additional-information`、`figure-legends`、`supplementary-materials`
- 若 `main.tex` 还通过 `\input{}` 或 `\include{}` 引入其它承载正文语义的 `.tex`，也要一并纳入
- 禁止把“当前文件里首次出现已定义”误判为通过；若该缩写在另一处正文 tex 中更早出现，则应以全篇首次出现位置为准

### 必做动作

1. 在真正落笔前，先通读并登记全文缩写，形成当前运行目录的 `analysis/abbreviation-inventory.md`
2. 对每个缩写至少记录：缩写形式、规范全称、全篇首次出现位置、允许的后续写法、已发现的不一致写法
3. 若准备新增一个缩写，先判断它是否会在全文复用并提升可读性；若只是当前局部方便，默认不要新造缩写
4. 修改任一局部 tex 后，回查全文 inventory，确保没有把同一概念写成两套缩写或两套全称
5. 在结束前，对纳入作用域的全部正文 tex 重新扫一遍，更新 `analysis/consistency-check.md`

### 通过标准

- 全篇首次出现必须按“全称 + 缩写”或等价清晰形式定义，而不是只在当前文件局部自洽
- 缩写一旦确定，后文必须稳定复用同一写法；同一概念不要在不同 tex 里来回切换
- 若某缩写只在单处出现或不能显著降低阅读负担，优先删掉缩写，直接保留全称
- Figure legend、Supplementary Materials 和 Abstract 中出现的缩写，也必须回到全文口径检查，不单独放宽
- 只检查当前正在编辑的 tex 文件，视为未完成该项审查

## 逻辑审查

论文必须被视为一个整体，不允许只修局部而破坏全局逻辑。

### 逻辑树

在写作或修订过程中，必须维护当前版本的逻辑树，至少包含：

- 核心问题
- 主线论点
- 次线论点
- 每个论点对应的关键证据
- 章节与论点之间的对应关系

逻辑树主文件保存为当前运行目录的 `analysis/logic-tree.md`，格式参考 `references/templates/logic-tree-template.md`。

### 多轮逻辑审查

每轮逻辑审查都要回答：

1. 主线是否清楚
2. 证据是否足以支撑论点
3. 章节顺序是否帮助读者理解
4. 是否存在断裂、矛盾、冗余、跳跃

### 审查机制

- 使用 `parallel-vibe` 做独立并行审查
- 并发参数与 SDK 以 `config.yaml:logic_check.parallel_vibe` 为准
- 每个 runner 都要独立重建逻辑树并指出问题
- 将结果写入当前运行目录的 `logic-check/`
- 每轮都使用 `references/templates/logic-check-template.md`
- 若任一 runner 发现阻塞性逻辑问题，就必须进入下一轮修复与复审
- 直到最新一轮逻辑树没有实质性问题，才能结束

## 全文一致性终审

至少检查：

- `Abstract`、`Results`、`Discussion` 关键数字是否一致
- 缩写首次定义是否完整且全文统一
- 缩写检查是否已经覆盖全部纳入作用域的正文 `.tex`，而不是只覆盖本轮编辑文件
- `analysis/abbreviation-inventory.md` 中登记的首次出现位置、规范写法与正文现状是否一致
- Figure/Table 编号、正文引用和图注是否一致
- Figure / Supplementary Figure legend 是否足以让非领域读者独立完成基本读图
- `Introduction` 提出的问题，`Results` 和 `Discussion` 是否有回应
- `Introduction` 是否已经去除无推进功能的核心观点、痛点或问题重复
- 非结构性冒号解释句是否过密，是否已改写为更自然的 SCI 论文句式
- 风格是否统一，是否仍保留作者特征
