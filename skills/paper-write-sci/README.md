...
- 普通正文尽量少用 `X: Y` 这类冒号解释句；除 Figure legend panel、列表、术语定义、方法字段或期刊格式要求外，优先写成自然从句、分号或独立句
- `Introduction` 会先安排核心痛点、空白、方法切入和贡献的各自落点；如果某句只是换词重复同一核心问题，默认合并、删除或改成新的证据/边界信息
- 终审时会额外检查冒号句密度和 `Introduction` 的主线重复，避免论文读起来像口头汇报提纲
