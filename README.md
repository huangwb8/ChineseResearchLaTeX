...
- **nsfc-reviewers**：模拟领域专家视角对 NSFC 标书进行多维度评审（创新性/可行性/基础与团队等），输出分级问题（P0/P1/P2）与可执行修改建议，并默认给出“基于当前版本直接送审”的函评/会评 `给过 / 不给过` 判断；每组固定 7 位专家，默认 3 组、最多 5 组，兼顾评审稳定性与调用成本
...
| [nsfc-reviewers](skills/nsfc-reviewers/) | v1.3.0 | 📝 日常 | NSFC 标书多专家多维度评审模拟（默认 3 组、最多 5 组，含函评/会评给不过判断） | ✅ 稳定 |
