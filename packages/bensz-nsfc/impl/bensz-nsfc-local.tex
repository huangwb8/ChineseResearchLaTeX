...
% Word 排版不会做"页高拉伸"，为稳定对齐（尤其是像素对比），禁用 LaTeX 的 flushbottom
\raggedbottom
...
\geometry{left=3.20cm,right=2.94cm,top=2.67cm,bottom=2.91cm} % 页边距（对齐 2026 Word 基准 PDF）
\CJKsetecglue{\hskip 0.15em plus 0.05em minus 0.05em} % 数字与 CJK 字符的间距
...
\ifwindows
  ...
  \setCJKfamilyfont{sectionzhfont}{KaiTi}[AutoFakeBold=5] % 定义模板中文字体为楷体
  ...
  \setCJKmainfont{KaiTi}[AutoFakeBold=5]  % 定义主体内容的字体为Kaiti
  \setCJKmonofont{KaiTi}[AutoFakeBold=5]  % 定义 CJK 等宽字体（避免 xeCJK tt 字体警告）
  \setmainfont{Times New Roman} % 定义主体内容的英文字体为Times New Roman
  % \setCJKmainfont{SimSun}[AutoFakeBold=5]  % 定义主体内容的字体为SimSun
  \xeCJKsetup{PunctStyle=quanjiao} % 强制中文标点符号使用中文字体
\else
  \ifNSFCHasBundledFonts
    \setCJKfamilyfont{sectionzhfont}[Path={\NSFCResolvedFontsDir}, AutoFakeBold=5]{Kaiti.ttf} % 定义模板中文字体为楷体
    \newcommand{\sectionzhfont}{\CJKfamily{sectionzhfont}} % 定义模板中文字体为新命令\sectionzhfont
    \newfontfamily\sectionenfont[Path={\NSFCResolvedFontsDir}, AutoFakeBold=5]{Kaiti.ttf} % 定义模板中英文字体为楷体，并定义新命令\sectionzhfont
    \NewDocumentCommand \templatefont {} {\sectionzhfont \sectionenfont} % 模板中英文字体
    % \setmainfont[Path={\NSFCResolvedFontsDir}, Extension=.ttf]{TimesNewRoman} % 定义主体内容的英文字体为Times New Roman。由于是外挂字体，无法自由使用英文斜体和粗体。
    % \setCJKmainfont[Path={\NSFCResolvedFontsDir}, Extension=.ttf, AutoFakeBold=5]{SimSun} % 定义主体内容的字体为SimSun
    \setCJKmainfont[Path={\NSFCResolvedFontsDir}, AutoFakeBold=5]{Kaiti.ttf} % 定义主体内容的字体为Kaiti
    \setCJKmonofont[Path={\NSFCResolvedFontsDir}, AutoFakeBold=5]{Kaiti.ttf} % 定义 CJK 等宽字体（避免 xeCJK tt 字体警告）
    \setmainfont[BoldFont=Times New Roman, AutoFakeBold=5]{Times New Roman} %使用MacOS自带的Times New Roman，可以自由使用英文斜体和粗体。
    \xeCJKsetup{PunctStyle=quanjiao} % 强制中文标点符号使用中文字体
  \else
    \setCJKfamilyfont{sectionzhfont}{KaiTi}[AutoFakeBold=5] % 定义模板中文字体为楷体
    \newcommand{\sectionzhfont}{\CJKfamily{sectionzhfont}} % 定义模板中文字体为新命令\sectionzhfont
    \newfontfamily\sectionenfont{KaiTi}[AutoFakeBold=5] % 定义模板中英文字体为楷体，并定义新命令\sectionzhfont
    \NewDocumentCommand \templatefont {} {\sectionzhfont \sectionenfont} % 模板中英文字体
    \setCJKmainfont{KaiTi}[AutoFakeBold=5]  % 定义主体内容的字体为Kaiti
    \setCJKmonofont{KaiTi}[AutoFakeBold=5]  % 定义 CJK 等宽字体（避免 xeCJK tt 字体警告）
    \setmainfont{Times New Roman} % 定义主体内容的英文字体为Times New Roman
...
% 正文段首缩进：仅在 extraTex 正文中启用，避免影响 main.tex 的提示语排版
...
\setlist[enumerate]{
  ...
  before={\setlength{\hangindent}{2em}\hangafter=1} % enumerate 换行后的续行额外缩进 2 个中文字符
  % after=\vspace{5pt}
}
...
\titlespacing*{\section}
  ...
  {2pt} % Space after the title（用于减小与 subsection 的距离；避免依赖 \parskip）

% Define line stretch for subsection
\titlespacing*{\subsection}
  {2pt} % Left indentation
  {5pt plus 0pt minus 0pt} % Space before the title
