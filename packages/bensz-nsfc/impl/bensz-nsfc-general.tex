...
\else
  \ifNSFCHasBundledFonts
    % 优先使用公共包内共享字体资源，保持项目输出可复现且减少重复文件。
    ...
    \setCJKmonofont[Path={\NSFCResolvedFontsDir}, AutoFakeBold=5]{Kaiti.ttf}
...
\setlength{\parskip}{0pt} % LaTeX 默认：不使用段后距（避免与 titlesec 的标题间距叠加导致不一致）
\XeTeXlinebreaklocale "zh" % 中文断行

% Word 空白段落的“视觉高度”对齐：显式补 1 行行距（避免依赖 \parskip）
\newcommand{\NSFCBlankPara}{\vspace*{\baselineskip}}
...
\titlespacing*{\subsection}
  ...
  {7.8pt} % Space after the title（模拟 Word“段后 7.8pt”，且不依赖全局 \parskip）
...
\setlength{\NSFCBibTextWidth}{397.16727pt} % 按 Young 的"参考文献条目有效行宽"对齐，用于降低跨项目换行差异；需要更宽/更窄可自行调整
