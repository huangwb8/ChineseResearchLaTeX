...
- DOCX 后处理会把 `References` 自动重排到图注节之前，并同时兼容 `Figure legends` / `Figure titles and legends` 与 `Supplementary materials` / `Supplemental information titles and legends` 这两组尾部标题命名
