...
%------------------------ 颜色 ------------------------%
\definecolor{CasBlue}{RGB}{0,113,192}
...
%------------------------ 页面尺寸（A4） ------------------------%
% 《博士后研究报告编写规则》要求：天头 30mm，订口 35mm 以上，
% 地脚 25mm，切口 20mm 以上。这里按单面 A4 报告设置。
\geometry{left=3.5cm,right=2.0cm,top=3.0cm,bottom=2.5cm,headheight=14pt,headsep=10pt}
...
%------------------------ 超链接 ------------------------%
% 说明：
% - 正文中的 cite 引用（citecolor）与 URL 链接（urlcolor）使用 CasBlue
% - 目录中的页码/标题链接（linkcolor）默认也为 CasBlue，但在目录输出时局部覆盖为黑色
...
\newcommand{\casdocTOCBlackLinks}{%
  \hypersetup{linkcolor=black}%
}
\newcommand{\casdocTOCRestoreLinks}{%
  \hypersetup{linkcolor=CasBlue}%
}
...
%------------------------ 页眉页脚 ------------------------%
\newcommand{\casdocHeaderFont}{\zihao{-5}}
\newcommand{\casdocHeaderTextMain}{博士后研究工作报告}
...
\newlength{\casdocAbstractTopGap}
\setlength{\casdocAbstractTopGap}{0.08cm}

\fancypagestyle{casdoc-main}{
  \fancyhf{}
  \fancyhead[C]{\casdocHeaderFont \casdocHeaderTextMain}
  \fancyfoot[R]{\thepage}
  \renewcommand{\headrulewidth}{0.4pt}
  \renewcommand{\footrulewidth}{0pt}
}

\fancypagestyle{casdoc-front}{
  \fancyhf{}
  \fancyhead[C]{\casdocHeaderFont \casdocHeaderTextMain}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0.4pt}
...
\fancypagestyle{casdoc-abstract-zh}{
  \fancyhf{}
  \fancyhead[C]{\casdocHeaderFont 摘\hspace{0.5em}要}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0.4pt}
  \renewcommand{\footrulewidth}{0pt}
}

\fancypagestyle{casdoc-abstract-en}{
  \fancyhf{}
  \fancyhead[C]{\casdocHeaderFont ABSTRACT}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0.4pt}
  \renewcommand{\footrulewidth}{0pt}
}

\newcommand{\casdocUseFrontPageStyle}{%
  \pagestyle{casdoc-front}%
...
\newcommand{\casdocUseMainPageStyle}{%
...
\ctexset{
  chapter = {
    ...
    format = \centering\bfseries\sffamily\zihao{3},
  ...
  section = {
    format = \bfseries\sffamily\zihao{4},
    beforeskip = 8pt,
    afterskip = 6pt
  },
  subsection = {
...
%------------------------ 工具宏 ------------------------%
\newcommand{\casdocTOCAddChapter}[1]{%
  \phantomsection%
  \addcontentsline{toc}{chapter}{#1}%
}

\newcommand{\casdocMainChapter}[1]{%
  \clearpage
  \input{#1}%
}

\newcommand{\casdocAppendix}{%
  \appendix
...
\newcommand{\casdocCoverLinebreak}{\\}

% 摘要区块：左侧粗体标签 + 正文
\newcommand{\casdocAbsLabel}[1]{\noindent\textbf{#1}\ }
...
% - 用法示例：
%   \casdocBiCaption[tab:demo]{公开文化材料分组说明}{Grouping of public cultural materials}
\newcommand{\casdocZhCaption}[2][]{%
  \begingroup
...
% - 字号：\renewcommand{\casdocBibFontCmd}{\zihao{4}}
% - 行距：\renewcommand{\casdocBibStretch}{1.5}
% - 条目间距：\setlength{\casdocBibItemSep}{4pt}
% - 条目内段间距：\setlength{\casdocBibParsep}{0pt}

% 字号（默认五号）：比正文四号更紧凑，但保留正式报告的可读性
\newcommand{\casdocBibFontCmd}{\zihao{5}}
% 行距倍数：参考文献默认略紧凑，必要时可在 main.tex 中覆盖为 1.5
\newcommand{\casdocBibStretch}{1.2}
% 条目间额外间距（biblatex \bibitemsep）
\newlength{\casdocBibItemSep}
\setlength{\casdocBibItemSep}{2pt}
% 条目内段落间距（biblatex \bibparsep）
\newlength{\casdocBibParsep}
\setlength{\casdocBibParsep}{0pt}

% 在参考文献列表开始时自动应用上述参数
\AtBeginBibliography{%
  \casdocBibFontCmd
  \setstretch{\casdocBibStretch}%
  \setlength\bibitemsep{\casdocBibItemSep}%
  \setlength\bibparsep{\casdocBibParsep}%
}
...
%------------------------ 参考文献引用（位置与间距） ------------------------%
% 说明：
% - 推荐写法：把 \cite 放在中文标点之前，例如：……跨媒介传播路径逐渐清晰\cite{xxx}。
% - XeLaTeX + xeCJK 默认会在 CJK 与 ASCII（如 \cite 输出的 [1]）之间插入间距，导致“前列 [1]”；
%   这里在常用 cite 命令开头加入 \unskip，移除紧邻其前的空格/胶水，得到“清晰[1]”。
