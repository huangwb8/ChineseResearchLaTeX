\ProvidesFile{bthesis-style-thesis-jlau-master.tex}
  [2026/06/05 v1.0.3 Thesis style: thesis-jlau-master]
...
\newcommand{\jlauCoverTopFont}{\jlauHei\fontsize{12bp}{17bp}\selectfont}
...
\newcommand{\jlauCoverDateFont}{\jlauSong\bfseries\fontsize{14bp}{20bp}\selectfont}
...
\providecommand{\jlauWordmarkPath}{assets/branding/jlau-wordmark.png}
...
\newcommand{\jlauMetaLine}[2]{%
  {\jlauCoverInfoFont
    \makebox[3.0cm][r]{#1}\hspace{0.42cm}%
    \underline{\makebox[6.35cm][c]{#2}}\par}%
  \vspace{0.24cm}%
}

\newcommand{\jlauCoverCodeLine}[2]{%
  \makebox[4.75cm][l]{%
    \makebox[1.85cm][s]{#1}%
    \makebox[0.42cm][c]{：}%
    \makebox[2.48cm][l]{#2}%
...
\newcommand{\jlauCoverEmblemAt}[1]{%
  \begin{scope}[shift={(#1)}]
    \clip (0,0) circle[radius=1.38cm];
    \node[inner sep=0pt] at (0,0)
      {\includegraphics[width=2.84cm,height=2.84cm]{\jlauEmblemPath}};
  \end{scope}%
}

\newcommand{\jlauCoverWordmarkAt}[1]{%
  \IfFileExists{\jlauWordmarkPath}{%
    \node[anchor=north,inner sep=0pt] at (#1)
      {\includegraphics[width=8.2cm]{\jlauWordmarkPath}};
  }{}%
}
...
\newcommand{\jlauMakeCover}{%
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=east,inner sep=0pt]
      ...
      {{\jlauCoverTopFont \jlauCoverCodeLine{单位代码}{\jlauSchoolCode}}};
    \node[anchor=west,inner sep=0pt]
    ...
    \node[anchor=east,inner sep=0pt]
      ...
      {{\jlauCoverTopFont \jlauCoverCodeLine{学号}{\jlauStudentId}}};
    ...
    \node[anchor=north,inner sep=0pt,align=center,text width=11.4cm]
      at ([yshift=-16.70cm]current page.north)
      {%
