...
- 更新 [projects/thesis-ucas-doctor/scripts/export_docx.py](projects/thesis-ucas-doctor/scripts/export_docx.py)：默认优先从 `docs/official/` 发现参考 Word 模板，并修复真实示例中的 `equation + aligned` 数学块与 `\graphicspath{{assets/}}` 图片路径解析问题；导出中间稿补齐“目录/图表目录/参考文献”结构标题，显式校验 `--reference-doc` 路径存在性，质量报告中“章节存在：图表目录”通过时明确输出 `PASS/已检测`
- 更新 [projects/thesis-ucas-doctor/README.md](projects/thesis-ucas-doctor/README.md)：补充“主文档/书脊双构建命令”、DOCX 验收命令与 `docs/official/` 的“用户自行下载 + SHA256 校验”口径
