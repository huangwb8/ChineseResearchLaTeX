...
- 继续对齐 `thesis-nju-master` 与南京大学工程管理学院 2023.2 版论文规范：`packages/bensz-thesis/styles/bthesis-style-thesis-nju-master.tex` 现将正文、章节标题、摘要标题和图表题注收敛到官方附件标注的仿宋体系，封面根据 `\njuSetDegreeType{professional|academic}` 区分“申请硕士专业学位 / 申请硕士学位”与“专业学位类别（领域）/ 学科、专业名称”字段，并新增原创性声明、学位论文使用授权声明和出版授权书页面宏；`projects/thesis-nju-master/main.tex` 默认装配这些前后置材料，`projects/thesis-nju-master/README.md` 同步移除不存在的 `baseline.tex` 说明；同时将 `packages/bensz-thesis/package.json` 版本推进到 `p_v20260606.1`，确保安装器不会因旧版本号跳过此次样式更新。
