...
- **NSFC_Young（2026）**：加粗 `\section` 标题，收紧“提纲提示语”与首个 `\section` 的间距，减小 `\section` 与 `\subsection` 的间隙，进一步收紧段落间距（含 `\subsubsection` 后首段；避免 `\indent` 形成空段落造成额外空白），并在“（提纲不做限制，……”的子标题中微调首字水平位置（更贴近 Word 模板观感）
