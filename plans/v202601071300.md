...
| **用户引导生成** | 🆕 支持用户自定义叙事提示（narrative_hint），AI 根据提示编造合理示例 |
...
**设计理念**：
- **示例场景** ≠ 严肃场景：允许 AI 根据用户提示编造合理的叙事内容
- **用户引导**：通过 `narrative_hint` 参数，用户可以指定研究主题、方法、场景等
- **AI 自由度**：AI 在用户提示框架内发挥创造力，生成连贯的示例内容

**支持场景示例**：
- 🏥 医疗影像："生成一个关于深度学习在医疗影像分析中应用的示例"
- 🔬 材料科学："创建一个关于新型纳米材料合成与表征的示例"
- 🧪 临床试验："模拟一个多中心临床试验的设计与分析流程"
- 🤖 传统 ML："编写一个使用支持向量机进行分类的示例研究"
...
class FormatGuard:
    ...
    def __init__(self, project_path: Path, run_dir: Path):
        ...
        self.run_dir = Path(run_dir)  # 🆕 运行目录
...
**AI 生成内容示例**（基于用户提示）：
```
本研究提出了一种基于深度卷积神经网络的医学影像诊断方法。
如图 1 所示，{PLACEHOLDER:figures/zzmx-115.jpg} 展示了
所提出的 CNN 架构，包含 5 个卷积层和 3 个全连接层。
为了解决医疗数据标注不足的问题，我们采用了数据增强策略，
包括旋转、翻转和强度变换等方法。
根据文献 {PLACEHOLDER:references:zhang2023deep} 的研究，
我们的方法在肺结节检测任务上取得了更好的性能。
```
...
result = skill.execute(
    project_name="NSFC_Young",
    options={
        "narrative_hint": "创建一个关于新型纳米材料合成与表征的示例，包括 XRD、SEM 等表征方法"
    }
)
```
...
result = skill.execute(
    project_name="NSFC_Young",
    options={
        "narrative_hint": "模拟一个多中心临床试验的设计与分析流程，重点描述随机化和盲法实施"
    }
)
```
...
4. ✍️ 生成阶段（🆕 支持用户提示）
   - 用户提示："生成一个关于深度学习在医疗影像分析中应用的示例"
   - AI 生成叙述：
     "本研究提出了一种基于深度卷积神经网络的医学影像诊断方法。
      如图 1 所示，{PLACEHOLDER:figures/zzmx-115.jpg} 展示了
      所提出的 CNN 架构..."
   - **关键**：AI 根据用户提示调整了内容方向和风格
...
│   ├── <run_id>/                   # 每次运行的唯一 ID
│   │   ├── backups/                # 备份文件
│   │   │   └── extraTex_1.2.内容目标问题.tex.backup
│   │   ├── logs/                   # 日志文件
│   │   │   ├── execution.log       # 执行日志
│   │   │   ├── format_check.log    # 格式检查日志
│   │   │   └── compile.log         # 编译日志
│   │   ├── analysis/               # AI 分析结果缓存
│   │   │   ├── section_themes.json
...
# ========== 日志配置 ==========
# 🚫 已废弃，使用 run_management.logging 替代（避免污染项目目录）
# 旧配置会在项目根目录创建 .complete_example.log，已弃用
# 新配置将日志放在 runs/<run_id>/logs/ 中
# logging:
#   level: "INFO"
#   file: ".complete_example.log"  # ❌ 旧设计，会污染项目
```
...
**开发模式**：AI 辅助一次性完成（无需分阶段）

### 核心模块实现

- [ ] **目录结构**：创建完整的 skills/complete_example/ 目录结构
- [ ] **SKILL.md**：编写 Skill 定义文档
- [ ] **config.yaml**：创建配置文件（包含 run_management 配置）
- [ ] **LLM 客户端**：实现统一的 LLM 接口封装
- [ ] **ResourceScanner**：实现资源扫描器（硬编码）
- [ ] **FormatGuard**：实现格式守护器（硬编码，支持 run_dir）
- [ ] **SemanticAnalyzer**：实现 AI 语义分析器
- [ ] **AIContentGenerator**：实现 AI 内容生成器（支持 narrative_hint）
- [ ] **CompleteExampleSkill**：实现主控制器（包含 _create_run_directory）
- [ ] **工具函数**：实现 LaTeX 解析、BibTeX 解析等工具
- [ ] **运行目录管理**：实现 runs/<run_id>/ 目录结构和软链接

### 测试与验证

- [ ] **单元测试**：ResourceScanner、FormatGuard、SemanticAnalyzer、AIContentGenerator
- [ ] **集成测试**：完整工作流测试（preview/apply/report 模式）
- [ ] **AI 能力测试**：主题理解、资源推理、内容质量
- [ ] **格式保护测试**：验证备份和日志不在项目目录
- [ ] **实战测试**：在 NSFC_Young、NSFC_Generic 上测试

### 文档与示例

- [ ] **README.md**：编写使用说明
- [ ] **使用示例**：创建 basic_usage.py、advanced_usage.py
- [ ] **最佳实践**：编写配置和使用最佳实践文档
...
1. ✅ **计划已确认**
2. 一次性创建完整目录结构
3. 一次性实现所有核心模块（ResourceScanner、FormatGuard、SemanticAnalyzer、AIContentGenerator、CompleteExampleSkill）
4. 一次性编写测试用例
5. 一次性在 NSFC_Young 上实战测试
6. 一次性编写所有文档
7. 发布 v1.0.0
...
**计划版本**: v1.3
**最后更新**: 2026-01-07 15:30
**状态**: 已优化（用户自定义叙事提示 + 运行目录隔离 + AI 一次性开发模式）

> **说明**：本计划的变更历史记录在根级 [CHANGELOG.md](../CHANGELOG.md) 中
